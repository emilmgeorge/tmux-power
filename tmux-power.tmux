...
tmux_set @prefix_highlight_copy_mode_attr "fg=$TC,bg=$BG,nobold"
...
LS="#[fg=$G04,bg=$TC] $session_icon #S #[fg=$TC,bg=$G06]$rarrow"
...
tmux_set window-status-format         "#[fg=$BG,bg=$G06]$rarrow#[fg=$TC,bg=$G06] #I:#W#(printf '%%-1s\n' '#F' | tr '-' '<')#[fg=$G06,bg=$BG]$rarrow"
tmux_set window-status-current-format "#[fg=$BG,bg=$TC]$rarrow#[fg=$BG,bg=$TC,nobold] #I:#W#(printf '%%-1s\n' '#F')#[fg=$TC,bg=$BG,nobold]$rarrow"

# Window status style
tmux_set window-status-style          "fg=$TC,bg=$BG,none"
tmux_set window-status-last-style     "fg=$TC,bg=$BG,nobold"
tmux_set window-status-activity-style "fg=$TC,bg=$BG,nobold"
